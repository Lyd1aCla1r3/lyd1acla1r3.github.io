...
<!-- SUMMARY: The Transformer architecture enters its second layer, shifting its analytical focus from isolated vocabulary words to highly contextualized representations. By projecting these enriched vectors into new Query, Key, and Value spaces, the self-attention mechanism begins computing relationships between complex semantic clusters. The mechanism is finalized by enforcing causality through lower-triangular masking and normalizing the scores into a strict probability distribution via the softmax function. These calculated attention probabilities are then blended with the value matrix to extract highly refined, contextualized features. -->
